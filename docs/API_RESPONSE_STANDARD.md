...
Tài liệu này định nghĩa cấu trúc chuẩn duy nhất cho mọi HTTP Response trả về từ hệ thống API Denolith. Frontend (FE) chỉ cần căn cứ vào các interface dưới đây để bóc tách dữ liệu và xử lý lỗi một cách đồng nhất.

---

## 1. Cấu Trúc Trả Về Chung (The Generic Response Wrapper)

Tất cả các API (trừ những API trả về `204 No Content` hoặc file tĩnh) đều trả về một đối tượng JSON với cấu trúc TypeScript như sau:
...
Thường áp dụng cho: `GET /:id`, `POST /`, `PATCH /:id`, `DELETE /:id` (Soft Delete).
...
**Response:**
*Không có Body. HTTP Status Code là 204.* FE chỉ cần check HTTP Status `204` để biết là xóa cứng thành công.

---

### 2.4. Trả Về Lỗi (HTTP 400, 401, 403, 404, 500)
Mọi lỗi xuất phát từ hệ thống đều có `success: false` kèm theo mã HTTP tương ứng.
...
**Ví dụ Lỗi Validation (400 Bad Request từ Valibot):**
Hệ thống sẽ bóc tách lỗi đầu tiên và đưa vào `message`.
```json
{
  "success": false,
...
```
*(Trong các trường hợp phức tạp, backend có thể trả thêm trường `errors` chứa chi tiết từng field để FE map vào Form).*

---

## 3. Gợi ý cho Frontend (Axios Interceptor Example)

FE nên cài đặt một Interceptor (nếu dùng Axios) hoặc wrapper (nếu dùng Fetch) để xử lý gọn nhẹ cấu trúc này:
