...
import { LanguageService } from "../modules/language/language.service.ts";
...
class DIContainer {
  ...
  get mediaService() {
    if (!this._mediaService) {
      this._mediaService = new MediaService(prisma);
    }
  ...
  get languageService() {
    if (!this._languageService) {
      this._languageService = new LanguageService(prisma);
    }
